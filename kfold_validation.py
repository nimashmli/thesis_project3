...
def validate(model_name, emotion ,category, k , num_people= 23, run_dir=None) : 
    ...
    for test_person in pateints :
        ...
        if run_dir:
            import os
            from run_utils import ensure_dir
            fold_run_dir = os.path.join(run_dir, f"fold_{i}")
            ensure_dir(fold_run_dir)
        history = choose_model(model_name, emotion, category , test_person , fold_idx=i, run_dir=fold_run_dir)
        if i ==0 : 
